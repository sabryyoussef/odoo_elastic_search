#!/usr/bin/env python3
"""
Odoo 16 Source Code & Documentation Indexer
Indexes Odoo 16 files (community and enterprise) into Elasticsearch with chunking support.
"""
import os
import sys
...
CODE_INDEX = os.getenv("CODE_INDEX", "odoo16_code")
DOC_INDEX = os.getenv("DOC_INDEX", "odoo16_docs")
ODOO16_ROOT = os.getenv("ODOO16_ROOT")
...
def main():
    ...
    # Validate Odoo root
    if not ODOO16_ROOT:
        print("❌ ODOO16_ROOT environment variable not set")
        print("Usage: export ODOO16_ROOT=/path/to/odoo16 && python index_odoo16.py")
        return 1
    
    root_path = Path(ODOO16_ROOT)
    if not root_path.exists():
        print(f"❌ Odoo 16 root directory does not exist: {root_path}")
        return 1
    
    print(f"🎯 Indexing Odoo 16 from: {root_path}")
